...
import support.api.utils.StableArrayAdapter;
...
public class DynamicListView extends ListView {
    ...
    public void setColorItemView(){
        int position = getLastVisiblePosition();
        View selectedView = getChildAt(position);
        selectedView.setBackgroundColor(Color.argb(90, 50, 188, 183));
    ...
    private Bitmap getBitmapWithBorder(View v) {
        ...
        paint.setColor(Color.rgb(50, 188, 183));
    ...
    private void updateNeighborViewsForID(long itemID) {
        ...
        int lastpos = getLastVisiblePosition() + 1;
        int pos = position +1;
        if(lastpos == pos){
    ...
    private void touchEventsEnded() {
        ...
        StableArrayAdapter adapter = ((StableArrayAdapter) getAdapter());
        adapter.clear();
        adapter.mIdMap.clear();
        for (int i = 0; i < StableArrayAdapter.mListaVisita.size(); i++) {
            ObjectSeqVisita seq = StableArrayAdapter.mListaVisita.get(i);
            int seque = i + 1;
            seq.setSequencia(seque);
            adapter.addItem(seque + "ª - " + seq.getNome_cliente());
            adapter.mIdMap.put(adapter.getItem(i).toString(), seque);
        }
        ((BaseAdapter) getAdapter()).notifyDataSetChanged();
    ...
    public interface SwappableListAdapter {
        void swap(ArrayList a, int index1, int index2);
    }
